...
[source,shell,subs=attributes+,role="primary"]
.Linux/OSX
----
...
[source,shell,subs=attributes+,role="secondary"]
.Windows
----
mvn io.quarkus:quarkus-maven-plugin:{quarkus-version}:create \
    -DprojectGroupId=org.acme \
...
We recommend continuing the journey with the link:building-native-image.html[building a native executable guide], where you learn about the native executable creation and the packaging in a container.
